namespace McCheat.Loader;

using Microsoft.Extensions.Logging;
using McCheat.Core.Memory;
using McCheat.Core.Features;
using McCheat.Core.Overlay;
using McCheat.Core.Config;
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        log.LogInformation("waiting for javaw.exe ...");
        int pid = ProcessFinder.WaitForProcess("javaw", TimeSpan.FromSeconds(120));
        if (pid <= 0)
        {
        ...
        var profiles = new ProfileManager("minecraft-cheat");
        ...
        log.LogInformation("attaching overlay to 'Minecraft'");
        if (!overlay.AttachToWindow("Minecraft"))
